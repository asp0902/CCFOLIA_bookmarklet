...
(() => {
  ...
  function isCcfBgmMuteButtonActive(button) {
    ...
    if (/(volumeoff|volumemute|volume_off|\bmuted\b|unmute|À½¼Ò°Å\s*ÇØÁ¦|«ß«å?«Èú°ð¶|«ß«å?«Èñé)/i.test(stateText)) {
      return true;
    }
  ...
  function createCcfYoutubeBgmListRow(entryKey, entry) {
    ...
    row.innerHTML = [
      ...
      '      <button class="MuiButtonBase-root MuiIconButton-root MuiIconButton-edgeEnd MuiIconButton-sizeLarge css-1pvfj5s ccf-youtube-bgm-edit" tabindex="0" type="button" aria-label="YouTube BGM ÆíÁý">',
  ...
  function openCcfYoutubeBgmEditPopover(entryKey, anchor) {
    ...
    popover.innerHTML = [
      ...
      `    <button class="MuiButtonBase-root MuiIconButton-root MuiIconButton-colorPrimary MuiIconButton-sizeSmall css-11qx9u ccf-youtube-bgm-loop" tabindex="0" type="button" data-loop="${loop ? "1" : "0"}" aria-label="«ê«Ô?«È" aria-pressed="${loop ? "true" : "false"}" title="¹Ýº¹Àç»ý">`,
  ...
  function isCcfRoomSettingsDialog(dialog) {
    ...
    return /·ë\s*¼³Á¤|·ë\s*°³¿ä|·ë\s*µ¥ÀÌÅÍ|¸â¹ö\s*¸®½ºÆ®|·ë\s*º¹Á¦|«ë?«à\s*àâïÒ|«ë?«à\s*?é©|«ë?«à\s*«Ç?«¿|«á«ó«Ð?\s*«ê«¹«È|«ë?«à\s*ÜÜð²|«¿?«Ü\s*«â?«É/i.test(text);
  }
