...
(() => {
  ...
  const BGM_DIALOG_KEYWORD_RE = /BGM|bgm|external\s*file|file\s*url|youtube|YouTube|유튜브|외부\s*파일|파일\s*URL|音楽|サウンド|ループ|loop|volume/i;
  ...
  function openCcfYoutubeBgmEditPopover(entryKey, anchor) {
    ...
    popover.innerHTML = [
      ...
      `        <input data-index="0" aria-label="ボリューム" aria-valuenow="${volume01}" aria-orientation="horizontal" aria-valuemax="1" aria-valuemin="0" name="volume" type="range" min="0" max="1" step="0.05" value="${volume01}" style="border: 0px; clip: rect(0px, 0px, 0px, 0px); height: 100%; margin: -1px; overflow: hidden; padding: 0px; position: absolute; white-space: nowrap; width: 100%; direction: ltr;">`,
      '      </span>',
      '    </span>',
      `    <p class="MuiTypography-root MuiTypography-body1 css-9l3uo3 ccf-youtube-bgm-volume-value">${volume01}</p>`,
      `    <button class="MuiButtonBase-root MuiIconButton-root MuiIconButton-colorPrimary MuiIconButton-sizeSmall css-11qx9u ccf-youtube-bgm-loop" tabindex="0" type="button" data-loop="${loop ? "1" : "0"}" aria-label="リピート" aria-pressed="${loop ? "true" : "false"}" title="반복재생">`,
  ...
  function isCcfRoomSettingsDialog(dialog) {
    ...
    return /룸\s*설정|룸\s*개요|룸\s*데이터|멤버\s*리스트|룸\s*복제|ルーム\s*設定|ルーム\s*概要|ルーム\s*データ|メンバー\s*リスト|ルーム\s*複製|ターボ\s*モード/i.test(text);
  }
