...
(() => {
  ...
  // 표준 사본은 "log editor/index.html" 에 있으며, 변경 시
  ...
  function buildRuntimeEntryFromMessage(value, tabContext = null, ctx = {}) {
    ...
    const bodyHtml = buildRenderedMessageHtml({
      ...
      baseColor,
      roll20Macro: extracted?.envelope?.roll20Macro,
      roll20Background: extracted?.envelope?.roll20Background
    });
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      .ccf-log-complete-dialog {
        ...
        background: #fff;
        color: #202020;
        font: 14px/1.55 -apple-system, BlinkMacSystemFont, "Segoe UI", "Pretendard", "Noto Sans KR", sans-serif;
        box-shadow: 0 18px 42px rgba(0, 0, 0, 0.3);
      }

      .ccf-log-complete-title {
        margin: 0 0 12px;
        font-size: 16px;
        font-weight: 700;
      }

      .ccf-log-complete-summary,
      .ccf-log-complete-note {
        margin: 0 0 12px;
      }
      ...
      .ccf-log-complete-actions {
        display: flex;
        justify-content: flex-end;
      }

      .ccf-log-complete-confirm {
        min-width: 68px;
        padding: 7px 16px;
      ...
      .ccf-log-complete-confirm:focus-visible {
  ...
  function openCapybaraLogEditorTab() {
    ...
    // 확인 모달의 클릭 이벤트에서 호출되어 팝업 차단을 피하고, 핸드오프를 위해 참조를 유지한다.
  ...
  function openCapybaraLogEditorAfterConfirmation(totalTabs, totalMessages) {
    return new Promise((resolve) => {
      document.querySelector(".ccf-log-complete-backdrop")?.remove();

      const backdrop = document.createElement("div");
      backdrop.className = "ccf-log-complete-backdrop";
      const dialog = document.createElement("div");
      dialog.className = "ccf-log-complete-dialog";
      dialog.setAttribute("role", "alertdialog");
      dialog.setAttribute("aria-modal", "true");
      dialog.setAttribute("aria-labelledby", "ccf-log-complete-title");
      dialog.innerHTML = `
        <h2 class="ccf-log-complete-title" id="ccf-log-complete-title">카피바라 로그 출력이 완료되었습니다.</h2>
        <p class="ccf-log-complete-summary">탭: ${totalTabs}개<br>메시지: ${totalMessages}개</p>
        <p class="ccf-log-complete-note">확인을 누르면 로그 편집기를 엽니다.</p>
        <div class="ccf-log-complete-actions">
          <button type="button" class="ccf-log-complete-confirm">확인</button>
        </div>
      `;
      backdrop.appendChild(dialog);
      (document.body || document.documentElement).appendChild(backdrop);

      const confirmButton = dialog.querySelector(".ccf-log-complete-confirm");
      confirmButton?.addEventListener("click", () => {
        backdrop.remove();
        resolve(openCapybaraLogEditorTab());
      }, { once: true });
      confirmButton?.focus();
    });
  }

  async function handleCapybaraLogLaunch(originButton = null) {
    if (buttonState.busy) return;
    ...
    try {
      ...
      editorWin = await openCapybaraLogEditorAfterConfirmation(totalTabs, totalMessages);
      if (!editorWin) return;
  ...
  function buildRenderedMessageHtml({ text, formatRuns, alignRuns, blockStyle, baseColor, roll20Macro, roll20Background }) {
    ...
    return roll20Macro ? wrapper.outerHTML : wrapper.innerHTML;
  }
