...
(() => {
  ...
  const CCF_LOG_PACKAGE_SCRIPT_INFO = Object.freeze({
    ...
    version: getUserscriptVersion("0.0.20"),
  ...
  function openCapybaraLogEditorTab() {
    ...
    restoreRoomTabFocus(editorWin);
    return editorWin;
  }

  function restoreRoomTabFocus(editorWin = null) {
    try { editorWin?.blur?.(); } catch {}
    try { window.focus(); } catch {}
  }

  function focusCapybaraLogEditorTab(editorWin = null) {
  ...
  async function handleCapybaraLogLaunch(originButton = null, editorWin = null) {
    ...
    try {
      ...
      focusCapybaraLogEditorTab(editorWin);
    } catch (error) {
