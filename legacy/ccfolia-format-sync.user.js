...
(() => {
  ...
  console.info("[CCF NAR] format-sync IIFE entry v0.1.53 @", new Date().toISOString());
  ...
  const CCF_FORMAT_SYNC_SCRIPT_INFO = Object.freeze({
    ...
    version: getUserscriptVersion("0.1.53"),
    namespace: "https://greasyfork.org/users/Capybara_korea/ccf-format-sync"
  });
  ...
  function applySoftBlur(el, blurValue) {
    ...
    // 네이티브 Bl 스펙: 흐림 반경 = 값의 2배(5px → 10px). 메시지별 값은 별도 이름으로 둔다 —
    // 같은 이름을 인라인에 박으면 :root 로 준 전역 조절값이 인라인에 눌려 안 먹는다.
    el.style.setProperty("--ccf-blur-r-msg", `${radius * 2}px`);
