...
(() => {
  ...
  console.info("[CCF NAR] format-sync IIFE entry v0.1.49 @", new Date().toISOString());
  ...
  const CCF_FORMAT_SYNC_SCRIPT_INFO = Object.freeze({
    ...
    version: getUserscriptVersion("0.1.49"),
    namespace: "https://greasyfork.org/users/Capybara_korea/ccf-format-sync"
  });
    ...
    if (!document.getElementById("ccf-blur-reveal-style")) {
      ...
      style.textContent = `
        /* 네이티브 Bl 원본 방식: 투명 글자 + 좌우 이중 그림자(가로 번짐).
           ...
           조절 손잡이: --ccf-blur-c(그림자 색, 어두운 테마면 #ccc 등), --ccf-blur-r(번짐 반경),
           --ccf-blur-x(좌우 간격). 셋 다 el 인라인 또는 전역 :root 로 덮으면 된다. */
        [data-ccf-blurred="1"] {
          -webkit-text-fill-color: transparent !important;
          text-shadow:
            var(--ccf-blur-x, 5px) 0 var(--ccf-blur-r, 10px) var(--ccf-blur-c, black),
            calc(-1 * var(--ccf-blur-x, 5px)) 0 var(--ccf-blur-r, 10px) var(--ccf-blur-c, black) !important;
