...
(() => {
  ...
  console.info("[CCF NAR] format-sync IIFE entry v0.1.54 @", new Date().toISOString());
  ...
  const CCF_FORMAT_SYNC_SCRIPT_INFO = Object.freeze({
    ...
    version: getUserscriptVersion("0.1.54"),
    namespace: "https://greasyfork.org/users/Capybara_korea/ccf-format-sync"
  });
  ...
  function applySoftBlur(el, blurValue) {
    ...
    // 북마클릿 Bl 규칙: 흐림 반경 = 값의 2배(5px → 10px). 메시지별 값은 별도 이름으로 둔다 —
  ...
  function ensureBlurRevealHandler() {
    ...
    if (!document.getElementById("ccf-blur-reveal-style")) {
      ...
      style.textContent = `
        /* 북마클릿 Bl 방식: 투명 글자 + 좌우 이중 그림자(가로 번짐).
           기본값 = color:rgba(0,0,0,0); text-shadow:5px 0 10px black,-5px 0 10px black.
