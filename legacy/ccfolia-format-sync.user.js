...
(() => {
  ...
  console.info("[CCF NAR] format-sync IIFE entry v0.1.50 @", new Date().toISOString());
  ...
  const CCF_FORMAT_SYNC_SCRIPT_INFO = Object.freeze({
    ...
    version: getUserscriptVersion("0.1.50"),
    namespace: "https://greasyfork.org/users/Capybara_korea/ccf-format-sync"
  });
  ...
  function applySoftBlur(el, blurValue) {
    ...
    // 네이티브 Bl 스펙: 흐림 반경 = 값의 2배(5px → 10px).
  ...
  function ensureBlurRevealHandler() {
    ...
    if (!document.getElementById("ccf-blur-reveal-style")) {
      ...
      style.textContent = `
        /* 네이티브 Bl 원본 방식: 투명 글자 + 좌우 이중 그림자(가로 번짐).
           네이티브 스펙 = color:rgba(0,0,0,0); text-shadow:5px 0 10px black,-5px 0 10px black.
           조절 손잡이: --ccf-blur-c(그림자 색, 기본 #cccccc, 밝은 테마면 black 등), --ccf-blur-r(번짐 반경),
           --ccf-blur-x(좌우 간격). 셋 다 el 인라인 또는 전역 :root 로 덮으면 된다. */
        [data-ccf-blurred="1"] {
          -webkit-text-fill-color: transparent !important;
          text-shadow:
            var(--ccf-blur-x, 5px) 0 var(--ccf-blur-r, 10px) var(--ccf-blur-c, #cccccc),
            calc(-1 * var(--ccf-blur-x, 5px)) 0 var(--ccf-blur-r, 10px) var(--ccf-blur-c, #cccccc) !important;
...
(() => {
  ...
  function closeMyCharacterPanel(panel) {
    ...
    const closeButton = panel.querySelector('button svg[data-testid="CloseIcon"]')?.closest("button")
      // \uD55C\uAD6D\uC5B4/\uC77C\uBCF8\uC5B4/\uC911\uAD6D\uC5B4/\uC601\uC5B4 \uB77C\uBCA8 \uBAA8\uB450 \uB300\uC751 (\uB77C\uBCA8\uC774 \uB2E4\uB974\uBA74 \uBABB \uCC3E\uC544 \uD328\uB110\uC774 \uC5F4\uB9B0 \uCC44 \uB0A8\uB294\uB2E4).
      || panel.querySelector('button[aria-label="\uB2EB\uAE30"], button[aria-label="Close"], button[aria-label="\u9589\u3058\u308B"], button[aria-label="\u95DC\u9589"], button[aria-label="\u5173\u95ED"]');
  ...
  async function scrapeNamesFromMyCharacterPanel() {
    ...
    if (!opened) {
      // 못 읽었더라도 우리가 연 패널은 닫아준다.
      if (!existingPanel) await ensureMyCharacterPanelClosed(button);
      return [];
    }

    // 원래 닫혀 있던 패널이면 반드시 다시 닫는다 (새로고침할 때마다 패널이 열린 채 남았다).
    if (!existingPanel) await ensureMyCharacterPanelClosed(button, opened.panel);
    return opened.names;
  }

  // 닫기 버튼 → Esc → 그래도 안 닫히면 열었던 버튼을 다시 눌러 토글한다.
  // "내 캐릭터 목록"은 버튼으로 여닫는 패널이라 닫기 버튼이 없을 수 있다.
  async function ensureMyCharacterPanelClosed(toggleButton, panel = null) {
    closeMyCharacterPanel(panel || getMyCharacterPanel());
    if (await waitForCondition(() => !getMyCharacterPanel(), 400)) return true;

    if (toggleButton instanceof HTMLElement) {
      try { toggleButton.click(); } catch (error) { /* 토글 실패는 무시 */ }
      if (await waitForCondition(() => !getMyCharacterPanel(), 500)) return true;
    }
    return !getMyCharacterPanel();
  }
  ...
  function setNarratorScrapeHidden(hidden) {
    if (hidden) {
      ...
      style.setAttribute(SAFE_UI_ATTR, "1");
      // 캐릭터 선택 드롭다운이 잠깐 번쩍이는 것만 가린다.
      // 예전엔 [role="presentation"] 까지 잡았는데, 채팅 서랍(MuiDrawer-root)도
      // presentation 이라 서식 툴바·나레이션 팝오버까지 통째로 투명해졌다
      // ("나레이션 창이 안 열림"의 원인). 우리 UI 를 품은 컨테이너는 제외한다.
      style.textContent = `
        body > .MuiPopover-root:not(:has(.ccf-inline-toolbar)),
        body > .MuiMenu-root:not(:has(.ccf-inline-toolbar)) {
          opacity: 0 !important;
  ...
  // 스캔이 캐릭터 선택 버튼에 포커스·마우스 이벤트를 보내면 코코포리아 툴팁("캐릭터 선택")이
  // 뜬 채로 남는다. 스캔이 끝나면 벗어난 것처럼 알려 툴팁을 걷어낸다.
  function dismissCharacterButtonHover(btn) {
    if (!(btn instanceof HTMLElement)) return;
    try {
      for (const type of ["pointerout", "pointerleave", "mouseout", "mouseleave"]) {
        const Ctor = type.startsWith("pointer") && typeof PointerEvent === "function" ? PointerEvent : MouseEvent;
        btn.dispatchEvent(new Ctor(type, { bubbles: type.endsWith("out"), cancelable: true, view: window }));
      }
      if (document.activeElement === btn) btn.blur();
    } catch (error) { /* 툴팁 정리 실패는 무시 */ }
  }
  ...
  async function scrapeCharacterSelectionNames() {
    ...
    try {
      ...
      dismissCharacterButtonHover(btn);
    }
  }
  ...
  async function scrapeCharacterNames(forcePanelScan = false) {
    ...
    // 캐릭터 선택 목록은 네이티브 드롭다운을 실제로 열었다 닫아야 읽을 수 있다.
    // 팝오버를 열 때마다 그러면 화면이 번쩍이고 팝오버가 닫히기도 한다.
    // → 한 번 읽어두고 재사용하며, "새로고침"(forcePanelScan) 일 때만 다시 읽는다.
    let selectable = ccfCachedSelectableNames;
    if (forcePanelScan || !selectable.length) {
      try {
  ...
  function preparePayloadForSend(editor, options = {}) {
    ...
    // runs를 담은 ccr20 envelope이면 변환 결과물로 보고 손대지 않는다.
    // (runs가 빈 ccr20 envelope = 평문 나레이션 프로즈 — 기존대로 fs가 처리)
    if (
      decodedCurrent?.envelope?.source === "ccr20-roll20-desc" &&
      (
        decodedCurrent.envelope.roll20Macro === true ||
        (Array.isArray(decodedCurrent.envelope.formatRuns) && decodedCurrent.envelope.formatRuns.length > 0)
      )
    ) return true;
