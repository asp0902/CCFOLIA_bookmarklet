...
(() => {
  ...
  const STYLE_CSS = `
    ...
    * { box-sizing: border-box; }
    ...
    header h1 {
      margin: 0; font-size: 0.875rem; font-weight: 700; line-height: 1.57;
    ...
    sh.innerHTML = `
      <style>
        ...
        * { box-sizing: border-box; font-family: "Noto Sans KR","Noto Sans JP","Roboto",system-ui,sans-serif; }
        ...
        .show-head h2 + .show-head-btn { margin-left: 8px; }
        .show-head h2 {
          margin: 0; font-size: 1.0625rem; font-weight: 700; flex: 1; color: #fff;
          pointer-events: none;
        ...
        .show-head-btn[data-action="show-edit"] + .show-close { margin-left: -4px; }
  ...
  function rowPopupSend(handoutId, audienceKey) {
    ...
    if (!h) return;
    // 전체 대상 또는 자기 자신 대상일 때 GM 본인 화면에도 즉시 표시.
    // (자기 자신은 Firestore 신호에서 skip 되므로 로컬에서 띄워야 함)
    const me = state.data.myCharacter || "";
    const isBroadcast = audienceKey === ALL_KEY || audienceKey === "all" || audienceKey === "*";
    const isSelf = !!me && !!audienceKey &&
                   removeSpaces(audienceKey).toLowerCase() === removeSpaces(me).toLowerCase();
    if (isBroadcast || isSelf) {
      try { showHandoutModal(h, me); } catch (_) {}
    }
  ...
  // ===== 초기화 =====
  function init() {
    // 상단 @version 과 함께 올릴 것 — 여기가 콘솔에서 실제로 확인되는 값이다.
    console.info(`[ccf-handout] init — version ${CCF_HO_SCRIPT_INFO.version}`);
    bindRouteEvents();
    bindGlobalKeys();
    startMountObserver();
