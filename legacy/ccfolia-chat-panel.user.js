...
(() => {
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      /* 블러(Bl): 네이티브와 동일한 방식 — 투명 글자 + 좌우 이중 그림자로 번짐 효과.
         (format-sync 기본은 단일 currentColor 그림자라 블록처럼 보였음.) 클릭 시 해제. */
      #${PANEL_ID} [data-ccf-blurred="1"] {
        -webkit-text-fill-color: transparent !important;
        text-shadow: 5px 0 10px currentColor, -5px 0 10px currentColor !important;
        filter: none !important; }
      #${PANEL_ID} [data-ccf-blurred="1"][data-ccf-blur-revealed="1"] {
        -webkit-text-fill-color: inherit !important; text-shadow: none !important; }
  ...
  function openPanel() {
    ...
    // 블러 해제는 Ctrl+클릭만. 일반 클릭으로 열리는 format-sync 기본 동작을 캡처 단계에서 막는다.
    listEl.addEventListener("click", (e) => {
      const blurred = e.target.closest?.('[data-ccf-blurred="1"]');
      if (!blurred) return;
      e.stopPropagation();
      if (e.ctrlKey || e.metaKey) {
        if (blurred.getAttribute("data-ccf-blur-revealed") === "1") blurred.removeAttribute("data-ccf-blur-revealed");
        else blurred.setAttribute("data-ccf-blur-revealed", "1");
      } else {
        blurred.removeAttribute("data-ccf-blur-revealed");
      }
    }, true);
    ...
    const buildPalette = () => {
      ...
      for (const cmd of cmds) {
        ...
        item.addEventListener("click", (e) => {
          e.stopPropagation();
          // 클릭하면 바로 전송(굴림). 팝업은 유지해 연속으로 굴릴 수 있게 한다.
          if (sending) return;
    ...
    inputEl.addEventListener("keydown", (event) => {
      if (!suggestEl.hidden) {
        ...
        // 추천이 떠 있으면 Enter 는 항상 "채우기만" — 전송은 다음 Enter 로(추천 닫힌 뒤).
        if (event.key === "Enter" && !event.isComposing) {
          const items = suggestEl.querySelectorAll(".ccf-scp-suggest-item");
          if (items.length) {
            event.preventDefault(); event.stopPropagation();
            inputEl.value = items[suggestActive >= 0 ? suggestActive : 0].textContent;
            suggestEl.hidden = true;
            return;
          }
        }
  ...
  function init() {
    ...
    window.__CCF_SECOND_CHAT_PANEL__ = {
      ...
      // 우리 전송에서도 굴릴 수 있다.
      bcdiceDiag() {
        const out = { webpackKey: null, reqGot: false, cacheSize: 0, factorySize: 0, found: [] };
        ...
        try {
          ...
          out.reqGot = !!req;
          if (!req) return out;
          out.cacheSize = req.c ? Object.keys(req.c).length : 0;
          const m = req.m || {};
          const ids = Object.keys(m);
          out.factorySize = ids.length;
          // 팩토리 소스에서 BCDice 관련 키워드를 찾는다(로드하지 않고 정적 검색).
          for (const id of ids) {
            let src = "";
            try { src = Function.prototype.toString.call(m[id]); } catch (e) { continue; }
            if (/DynamicLoader|dynamicLoad|GameSystemClass|BCDice|bcdice/i.test(src)) {
              const hit = (src.match(/DynamicLoader|dynamicLoad|GameSystemClass|BCDice/gi) || []).slice(0, 4);
              out.found.push({ id, hits: [...new Set(hit)], len: src.length });
            }
            if (out.found.length > 20) break;
          }
